...
/**
 * Apply Section Metadata to sections on normal page load.
 * This project's aem.js decorateSections() does not consume `section-metadata`
 * blocks (only the Target mutation path in this file does), so a `style` value
 * like "highlight" would never become a section class on a standard load.
 * This mirrors the standard EDS behavior: read each `.section-metadata` block,
 * turn its `style` values into classes on the parent section, other keys into
 * dataset entries, then remove the metadata block.
 * @param {Element} main
 */
